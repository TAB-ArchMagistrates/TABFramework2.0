...
-Instructions
	 ...
	 2. Copy & Paste your API Key from https://trello.com/app-key into ]] "Here!"; 																																																				--[[
	 3. Authorize the application by substituting API-KEY in the link bellow with the api key from https://trello.com/app-key
			  https://trello.com/1/authorize?key=[API-KEY]&name=Roblox+Api&expiration=never&response_type=token&scope=read,write	
...
local suc,err=pcall(function()
	if game.Players.LocalPlayer~=nil then
		warn("It is unsafe to have the api in a local enviorment please move it to ServerScriptService and ONLY use it from Server Scripts!")
	end
end)
